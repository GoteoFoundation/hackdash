...
const redirectToSaved = ({session}, res) => res.redirect(session.redirectUrl || '/');

// Crreate a new token for Bearer authentication automatically
const buildBearer = async (req, res, next) => {
  const token = {name: "LoginAuto", token: uuid()};
  console.log('Adding token' , token);
  req.user.tokens = _.reject(req.user.tokens, function(t){
    return t.name === "LoginAuto";
  });
...
const setSocial = (user, profile) => {
  var changed = false;
  user.social = user.social || {};
  switch(profile.provider) {
    case 'facebook':
      if(!user.social.facebook) {
          user.social.facebook = 'https://facebook.com/' + profile.id;
          changed = true;
      }
      break;
    case 'github':
      if(!user.social.github) {
          user.social.github = profile.profileUrl || 'https://github.com/' + profile._json.login;
          changed = true;
      }
      break;
    case 'google':
      if(!user.social.google) {
          user.social.google = profile._json.url;
          changed = true;
      }
      break;
    case 'twitter':
      if(!user.social.twitter) {
          user.social.twitter = 'https://twitter.com/' + profile._json.screen_name;
          changed = true;
      }
      break;
  }
  // console.log('setSocial', profile, user.social.toString());
  return changed;
};
...
const generateStrategy = provider => {
  ...
  app.get(`/auth/${provider}/callback`, passport.authenticate(provider, { failureRedirect: '/login', failureFlash: true }), redirectToSaved);
  ...
  passport.use(new Strategy(ops, async (req, token, tokenSecret, profile, done) => {
    try {
      ...
      if(!user) {
        ...
        // TODO: Send welcome message if has email
        if(user.email) {
        ...
        setPicture(user, profile);
        var changed = setSocial(user, profile);
        // social data
        if (user.picture !== picBefore || changed){
...
if(config.useLocalLogin) {
  ...
  const localRegister = async (req, res, next) => {
    ...
    try {
      ...
      req.flash('error', 'User cannot be created due a server error: ', err.message);
  ...
  app.post('/embed/login',
    saveRedirect,
    passport.authenticate('local', { failureRedirect: '/embed/login', failureFlash: true }),
    buildBearer,
    ({session}, res) => res.redirect(session.redirectUrl || '/embed/login'));

  app.post('/register', saveRedirect, localRegister, buildBearer, redirectToSaved);

  app.post('/lost-password', saveRedirect, localNewToken, redirectToSaved);
