...
export const sendNewCommentMail = ({to,project,from,comment}, callback) => {
  if(!_.isFunction(callback)) callback = errorHandler;
  transport.sendMail({
    from: mailerFrom,
    to: to.email,
    subject: sWrapper(__('template_new_comment')),
    html: wrapper(_.template(__('template_new_comment_body'))({
      host: publicHost,
      name: to.name,
      comment: comment,
      from: from.name,
