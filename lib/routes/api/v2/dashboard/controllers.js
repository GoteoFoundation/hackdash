...
import {maxQueryLimit, publicDashboardCreation} from 'config';
...
const projectToCSV = project => {
  ...
  const addPerson = (engagement, user) => {
    if(project.leader._id.toString() != user._id.toString() || engagement == 'leader') {
      people.push([user.name, user.username, user.provider, user.email, engagement, project.title, project.status, project.domain]);
    }
  };
