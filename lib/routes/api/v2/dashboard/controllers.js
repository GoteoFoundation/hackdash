...
import {maxQueryLimit, publicDashboardCreation, publicHost} from 'config/config';
import {CSVEscape} from '../helpers';

const maxLimit = maxQueryLimit || 50;

export const validateSubdomain = (req, res, next) => {
  if(!(publicDashboardCreation) && !req.user.superadmin) {
...
export const isOwnerDashboard = async ({user, dashboard}, res, next) => {
  if (!dashboard.owner) {
    return res.status(403).send('This event board cannot be removed because it has no owner.');
  }

  if (dashboard.owner._id.toString() !== user._id.toString()) {
    return res.status(403).send('Only Owner can remove this event board.');
  }

  if (dashboard.projectsCount > 0) {
    return res.status(403).send('Only Events board with no projects can be removed.');
  }

  const count = User.count({ admin_in: dashboard.domain });
  if (count > 1){
    return res.status(403).send('Only Events board with ONE admin can be removed.');
  }
  next();
};
...
export const removeDashboard = async ({dashboard}, res) => {
  ...
  try {
    ...
    res.status(500).send('An error ocurred when removing this event board');
  }
...
const projectToCSV = project => {
  ...
  const addPerson = (engagement, user) => {
    if(project.leader._id.toString() != user._id.toString() || engagement == 'leader') {
      people.push([user.name, user.username, user.provider, user.email, engagement, project.title, project.status, project.domain, project.cover ? publicHost + project.cover : '']);
    }
  };

  addPerson('leader', project.leader);
  project.contributors.forEach(addPerson.bind(null, 'contributor'));
  project.followers.forEach(addPerson.bind(null, 'follower'));
...
export const sendDashboardCSV = ({dashboard}, res) => {
  ...
  const headers = ['name', 'username', 'provider', 'e-mail', 'engagement',
                    'project', 'status' ,'dashboard', 'cover'].map(CSVEscape).join(',');
