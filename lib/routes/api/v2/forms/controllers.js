...
import {Collection, Form, Dashboard, Project, User} from 'lib/models';
...
export const getForm = (req, res, next) => {
  ...
  Form.findById(req.params.fid)
    ...
    .exec(function(err, form) {
      ...
      req.form = form;
      if(form.public) {
        // public forms can be accessed by anyone registered
        next();
        return;
      }
      var domains = [];
...
const assignResponses = function(req, forms, project, user) {
  req.responses = req.responses || [];
  let form = req.form.toJSON();
  // console.log(form.questions);
    let resps = _.find(forms, function(f){
...
export const getResponses = async (req, res, next) => {
  // If public, no projects
  if(req.form.public) {
    User.find({"forms.form": req.form._id})
    .select(userPVT + ' email forms')
    .exec(function(err, users){
      if (err) return res.status(500).send(err.toString());
      if (!users) return res.status(404).send('No users found');

      _.each(users, function(u, i){
        assignResponses(req, u.forms, null, _.omit(u, 'forms'));
      });
      next();
    });
    return;
  ...
  Project.find(q)
    ...
    .exec(function(err, projects) {
      ...
      _.each(projects, function(p, i){
        ...
        assignResponses(req, p.forms, _.omit(p, 'forms'));
      });
      next();
    });
};
...
export const saveResponse = (req, res, next) => {
  ...
  try {
    var pform = {form: req.form._id, responses: []};
    var model = req.form.public ? req.user : req.project;
    model.forms = model.forms || [];

    var preform = _.find(model.forms, function(el) {
      // console.log("CHECK EXISTING FORMS", el.form, req.form._id);
      return el.form.toString() === req.form._id.toString();
    });
    var pform = preform || pform;
    ...
    if(!preform) {
      // console.log('PUSHING NEW RESULT');
      model.forms.push(pform);
    }
    // console.log('RESULT', 'public', req.form.public, 'form', pform.form, 'responses', pform.responses, 'model.forms', model.forms);
    model.save(function(err, model){
      if(err) return res.status(500).send(err);
      req.response = pform;
      // console.log('SAVED', model.forms);
      next();
    });
  } catch(e) {
...
export const sendResponses = (req, res, next) => {
  if(req.query.hasOwnProperty('csv')) {
    ...
    const headers = ['userId', 'userURL', 'name', 'e-mail', 'projectID', 'projectURL', 'project', 'dashboard', 'form', 'question', 'type', 'response'];
    ...
    let data = [headers];
    req.responses.forEach(({project, user, responses}) => {
      req.form.questions.forEach((q) => {
        let r = _.find(responses, (re) => re.question.toString() == q._id.toString()) || {};
        const resp = [
          user ? user.userId : project.leader.userId,
          publicHost + '/users/' + (user ? user._id : project.leader._id),
          user ? user.name : project.leader.name,
          user ? user.email : project.leader.email,
          project && project.projectId,
          project ? (publicHost + '/projects/' + project._id) : null,
          project && project.title,
          user ? req.form.domain : project.domain,
          req.form.title,
          q.title,
          q.type,
          format(q, r.value)];
