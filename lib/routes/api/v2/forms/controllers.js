...
const userPVT = '_id userId picture username name created_at';
...
export const sendResponses = (req, res, next) => {
  if(req.query.hasOwnProperty('csv')) {
    ...
    req.responses.forEach(({project, responses}) => {
      responses.forEach((r) => {
        const resp = [project.leader.userId, project.leader.name, project.leader.email, project.projectId,  project.title, project.domain, req.form.title, r.question.title, r.question.type, format(r.question, r.value)];
        data.push(resp);
      });
    });
