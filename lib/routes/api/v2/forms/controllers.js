...
export const getForm = (req, res, next) => {
  function _getProjects (domains) {
    Project.find({domain: {$in: domains}})
      .select('-__v')
  ...
  Form.findById(req.params.fid)
    ...
    .exec(function(err, form) {
      ...
      var domains = [];
      if(form.group) {
        Collection.find({_id: form.group})
          .exec(function(err, col) {
            if(err) return res.status(500).send(err.toString());
            if(col) {
              Dashboard.find({_id: {$in: col.dashboards}})
                .exec(function(err, dashboards) {
                  _getProjects(_.pluck(dashboards, 'domain'));
                });
            }
          });
      } else {
        _getProjects([form.domain]);
      }
  ...
  if(domain) {
    ...
    Collection.findOne({ _id: group })
      .exec(function(err, collection) {
        if(err) return res.status(500).send(err.toString());
        if(!collection) return res.status(404).send('No collection found');
        Dashboard.find({_id: {$in: _.pluck(collection, 'dashboards')}})
          .exec(function(err, dashboards) {
            if(err) return res.status(500).send(err.toString());
            if(!dashboard) return res.status(404).send('No dashboard found');
...
export const deleteForm = (req, res) => {
  ...
  if(req.form.open) {
    res.status(403).send('Error, please close this form before delete');
  }
  ...
  Project.find({'forms.form': req.form._id})
    .exec(function(err, projects) {
      ...
      if(projects && projects.length) {
        res.status(403).send('Error, this form cannot be deleted. Some projects already have responded it!');
      } else {
...
export const sendFile = (req, res) => {
  if (!req.file || req.file.fieldname !== 'file'){
    res.status(400).send({ error: 'file-field-expected' });
    return;
  }
...
export const deleteFile = (req, res) => {
  ...
  fs.unlinkSync('./public/' + req.body.file.path, function(err) {
    if(err) {
