...
const assignResponses = function(req, forms) {
...
export const getResponses = async (req, res, next) => {
  ...
  if(req.form.public) {
    User.find({"forms.form": req.form._id}).exec(function(err, users){
      _.each(users, function(u, i){
        assignResponses(req, u.forms);
      });
      next();
    });
  }
    ...
    req.responses.forEach(({project, responses}) => {
      req.form.questions.forEach((q) => {
        ...
        const resp = [
          ...
          publicHost + '/users/' + project.leader._id,
          project.leader.name,
          project.leader.email,
          project.projectId,
          publicHost + '/projects/' + project._id,
          project.title,
          project.domain,
          req.form.title,
          q.title,
