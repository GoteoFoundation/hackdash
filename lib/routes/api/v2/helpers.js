...
export const userAdminAndPermission = (user, domain, group, perm) => {
  ...
  if(domain && admin_in.indexOf(domain) == -1) {
    return false;
  }
  // if group(collection), needs to be admin in it
  if(group && group_admin_in.indexOf(group) == -1) {
    return false;
  }
  console.log('IsAdmin');
...
export const isAuth = (req, res, next) => req.isAuthenticated() ? next(): res.status(403).send('User not authenticated');
