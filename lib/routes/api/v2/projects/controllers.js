...
const userPVT = '_id userId picture username name role bio created_at';
const maxLimit = maxQueryLimit || 50;

const notify = (type, req) => {
  bus.emit('project_notification', {
    type: type,
    project: req.project,
    user: req.user,
...
export const cleanProject = async (req, res, next) => {
  let project = req.project;
  let isLeader = req.user && project.leader && project.leader._id && req.user && req.user._id && project.leader._id.toString() == req.user._id.toString();

  // Get the dashboard privacy status
  let privateBoard = await Dashboard.findOne({domain: project.domain}).select('private');
  privateBoard = privateBoard && privateBoard.private;
  // project.private = project.private || (privateBoard && privateBoard.private);
  // Check responses permission
  if(!isLeader) {
    if(!userAdminAndPermission(req.user, project.domain, project.group, ['project_edit', 'dashboard_view_private'])) {
      console.log(req.user && req.user.name,'HAS NO PERMISSION');
      if((project.private || privateBoard) && !userHasPermission(req.user, 'project_view')) {
        project.location = {};
        project.extra = { step0: project.extra && project.extra.step0 };

        project.link = null;
      }
    }
    if(!userAdminAndPermission(req.user, project.domain, project.group, 'form_results')) {
      console.log('remove responses due lack of permission', project.title);
...
export const createProject = (req, res, next) => {
  ...
  projectInDomain(project, function(err) {
    ...
    project.save(function(err, project){
      console.log('projectInDomain', err, project);
...
export const updateProject = (req, res, next) => {
  ...
  project.private = getValue("private");
...
export const removeProject = (req, res) => {
  ...
  req.project.remove(function (err){
    ...
    updateDashboard(domain, function(l){
      console.log('Updated dashboard',l);
      res.sendStatus(204); //all good, no content
    });
  });
};
...
export const validate = (req, res, next) => {
  ...
  if (user._id === project.leader.id ){
    return res.status(406).send("Leader of the project cannot leave or unfollow.");
  }
...
export const setQuery = async (req, res, next) => {
  ...
  if (query.length === 0) {
    if (!req.search_query.hasOwnProperty('domain') && !req.query.hasOwnProperty('include_empty')){
  ...
  var private_dashs = [];
  var hide_private_projects = {};
  if(!userHasPermission(req.user, 'project_view')) {
    ...
    private_dashs = _.pluck(dashboards, 'domain');

    hide_private_projects = {$or: [
      {private: false},
      {leader: req.user ? req.user._id : null}
    ]};
    console.log('HIDING PRIVATE PROJECTS', hide_private_projects);
  }
  // console.log('query1', req.search_query, private_dashs);
  req.search_query = {
    $and: [
      req.search_query,
      {$or: [
      ...
      ,hide_private_projects
    ]
  };
  console.log('setQuery', req.search_query);
  next();
};
