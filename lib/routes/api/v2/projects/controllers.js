...
export const activeStatuses = _.pluck(_.filter(statuses, function(s) {
    return s.active;
  }), 'status');
...
export const createProject = (req, res, next) => {
  ...
  // Check status active
  if(_.indexOf(activeStatuses, project.status) < 0) {
    return res.status(500).json({ error: "status_invalid" });
  }
