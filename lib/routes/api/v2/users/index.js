...
var setQuery = function(req, res, next){
  ...
  if (query.length === 0 && !req.query.hasOwnProperty('all')){
...
var getUsers = async function (req, res, next){
  ...
  if (req.isLanding){
    if(config.homeTabOrder === 'random') {
      var total = await User.count();
      offset = Math.min(total - limit, Math.floor(Math.random() * total));
      console.log('TOTAL USERS', total, offset);
    }
    else if(config.homeTabOrder === 'first_created')
      sort = "created_at " + sort;
    else if(config.homeTabOrder === 'first_created')
      sort = "-created_at " + sort;
  }
  console.log('LIMIT',req.limit, maxLimit, 'offset', offset);
  User
