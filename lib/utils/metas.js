...
var i18n = require('client/app/locale');
...
var hdDomain = config.publicHost;
var baseURL = hdDomain;

i18n.setLocale(config.language || 'en');

if(config.theme) {
  try {
    var locales = require('themes/' + config.theme + '/locale');
    // console.log("custom locales for metas " +  config.theme, locales);
    i18n.addLocales(locales);

  } catch(e) {
    // console.warn("No custom locales for metas " + config.theme, e);
  }
}
...
export const dashboard = function(req, res, next){
  ...
  if (!domain) {
    ...
    res.locals.meta = {
      url: baseURL,
      title: i18n.__("title_default"),
      description: i18n.__("description_default")
    };
...
export const check = function(){
  return function(req, res, next) {
    ...
    if (!metas.image){
      metas.image = config.image ? config.image : "/images/logohack.png";
    }
    ...
    console.log(baseURL, metas.image);
    next();
  };
};
